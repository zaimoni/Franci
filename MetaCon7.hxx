...
{	// specializations must provide exact_type member or else
};
...
protected:
	virtual ~MetaConceptExtern() {};	// as a template doesn't go in Destruct.cxx
public:
	...
	void operator=(const MetaConceptExtern& src) {
	...
	virtual void MoveInto(MetaConcept*& dest);	// can throw memory failure.  If it succeeds, it destroys the source.

//  Type ID functions
	virtual const AbstractClass* UltimateType() const;	// specialize or else
//	Arity functions
