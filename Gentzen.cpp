...
//	std::wcout << std::filesystem::canonical(who_am_i.native()) << "\n";
	...
	while (++idx < argc) {
		...
		kuroda::parser<lex_node>::symbols stage;
		auto wrapped = lex_node::pop_front(lines);
		while (wrapped) {
			stage.push_back(wrapped.release());
			wrapped = lex_node::pop_front(lines);
		};
		std::cout << std::to_string(stage.size()) << "\n";

		lex_node::to_s(std::cout, stage) << "\n";
	}
