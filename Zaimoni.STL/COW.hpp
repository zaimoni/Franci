...
private:
	auto _w_clone() const {
		if constexpr (requires() { _write->clone(); }) {
			return std::unique_ptr<T>(_write->clone());
		} else if constexpr (std::is_copy_constructible_v<T>) {
			return std::unique_ptr<T>(new T(*_write));
		} else {
			static_assert(unconditional_v<bool, false, T>, "unclear how to write-clone type");
		}
	}
	...
	void _rw_clone() {
		if constexpr (requires() { _read->clone(); }) {
			...
			static_assert(unconditional_v<bool, false, T>, "unclear how to read/write-clone type");
		}
		_read.reset();
	}
