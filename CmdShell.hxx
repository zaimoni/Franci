...
public:
	...
	// the input buffer being manipulated below, is allocated with C library memory

	CmdShellIOHook* StdOutHook;			// for the help/? pseudocommand
	CmdShellHandler* GetLineHookNoFail;	// get_line_to_interpret; incoming C string reference is NULL
	CmdShellHandler2* GetLineHook;		// get_line_to_interpret; incoming C string reference is NULL
	...
	CmdShell();
	CmdShell(const CmdShell& Source) = delete;	// not copyable
	CmdShell(CmdShell&& Source) = delete;
	void operator=(const CmdShell& Source) = delete;
	void operator=(CmdShell&& Source) = delete;
	virtual ~CmdShell();
...
// TODO: AdjustCommandHandler_do(char*& new_command_name, CmdShellHandler2* new_do_command)
	...
	void cmdstep();	// extension beyond Python cmd: lets us iterate command once
	void cmdloop();
	void emptyline();
	bool default_handler(char*& InputBuffer);
	bool postcmd(bool logical_continue, char*& InputBuffer);

	const char* lastcmd();					// last command
private:
	void preloop();
